...
def load_cache() -> Dict:
    if Path(CACHE_FILE).exists():
        with open(CACHE_FILE, "r") as f:
            return json.load(f)
    # next_start_index tracks where to begin the next API page fetch
    ...
    for psc in pscs:
        ...
        if incorporation_date and notified:
            if (notified - incorporation_date).days < 30:
                continue  # Original structure — not an acquisition
...
def analyse_company(client: CompaniesHouseClient, company_item: Dict, years_back: int) -> List[Dict]:
    ...
    for acquirer in acquirers:
        acquirer_name = acquirer.get("name") or (acquirer.get("identification") or {}).get("legal_name") or "Unknown"
        for founder in founders:
            ...
            rows.append({
                ...
                "acquirer_country":      (acquirer.get("identification") or {}).get("country_registered") or (acquirer.get("address") or {}).get("country") or "",
...
# ---------------------------------------------------------------------------
# Main screening loop — uses persisted start_index to advance each run
# ---------------------------------------------------------------------------

def get_next_companies(client: CompaniesHouseClient, sic_codes: List[str],
                       batch_size: int, start_index: int) -> Tuple[List[Dict], int]:
    """
    Fetch `batch_size` companies beginning at `start_index`.
    Returns the companies fetched and the new start_index to persist.
    """
    companies = []
    ...
    while len(companies) < batch_size:
        ...
        if len(items) < fetch_size:
            break  # Reached end of API results
...
def run_screen(api_key: str, batch_size: int, years_back: int, sleep_seconds: float) -> Tuple[pd.DataFrame, Dict]:
    ...
    # Always start from where the last run left off
    ...
    # Filter to only ones not yet screened (handles edge case of overlapping pages)
    ...
    if to_screen:
        ...
        st.info(
            f"All {total_fetched} companies in this batch were already in the cache. "
...
with st.sidebar:
    ...
    _cache = load_cache()
    st.divider()
    st.caption(f"📍 Next run starts at company index: **{_cache.get('next_start_index', 0)}**")
    st.caption(f"✅ Total screened to date: **{len(_cache.get('screened', {}))}**")

    st.divider()
    if st.button("🗑️ Clear cache & restart from index 0", type="secondary"):
        if Path(CACHE_FILE).exists():
...
3. At least one **corporate-entity PSC** holds 25%+ of shares AND was notified **more than 30 days after incorporation** — ruling out original holding structures.

**Pagination:** Each run fetches the next unseen batch from Companies House. The cursor advances automatically so no company is ever double-screened across runs.
    """)
...
if run:
    if not api_key:
        ...
        st.subheader("Screening summary — this run")
        cols = st.columns(6)
