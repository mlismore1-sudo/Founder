...
def mark_screened(cache: Dict, company_number: str, rows: List[Dict]) -> None:
    ...
    if rows:
        existing = {
            (r["company_number"], r["founder_name"], r["acquirer_company_name"])
            for r in cache["hits"]
        }
        for row in rows:
...
class CompaniesHouseClient:
    ...
    def advanced_company_search(self, sic_codes: List[str], size: int = 100, start_index: int = 0) -> Dict:
        # CH advanced search requires sic_codes as repeated params, not comma-joined
        params = [("size", size), ("start_index", start_index), ("company_status", "active")]
    ...
    def list_pscs(self, company_number: str) -> List[Dict]:
        result = self._get(f"/company/{company_number}/persons-with-significant-control")
...
def analyse_company(client: CompaniesHouseClient, company_item: Dict, years_back: int) -> List[Dict]:
    ...
    ch_url = "https://find-and-update.company-information.service.gov.uk/company/" + company_number
    ...
    for acquirer in acquirers:
        ...
        for founder in founders:
            ...
            dob_str = "/".join(filter(None, [month, year]))
            rows.append({
...
def get_next_companies(
    ...
    while len(companies) < batch_size:
        ...
        data = client.advanced_company_search(
            sic_codes=sic_codes,
            size=fetch_size,
...
def run_screen(
    ...
    if not to_screen:
        st.info(
            "All "
            + str(total_fetched)
            + " companies in this batch were already cached. "
            + "The next run will begin at index "
            + str(new_start_index)
            + "."
        )
    ...
    if not df.empty:
        ...
        df = df.sort_values(
            by=["acquirer_notified_on", "date_of_creation"],
...
with st.sidebar:
    ...
    _secrets = st.secrets.get("companies_house") or {}
    _default_key = _secrets.get("api_key", "") or os.getenv("CH_API_KEY", "")
    ...
    _cache = load_cache()
    st.divider()
    st.caption("Next run starts at index: **" + str(_cache.get("next_start_index", 0)) + "**")
    st.caption("Total screened to date: **" + str(len(_cache.get("screened", {}))) + "**")
    st.divider()
...
with st.expander("How signals are detected", expanded=False):
    st.markdown(
        ...
        "2. At least one **individual PSC** holds 25%+ of shares and was notified "
        "within 2 years of incorporation, or has since ceased.\n"
        "3. At least one **corporate-entity PSC** holds 25%+ of shares AND was notified "
        "**more than 30 days after incorporation** — ruling out original holding structures.\n\n"
        "**Pagination:** Each run fetches the next unseen batch. The cursor advances "
        "automatically so no company is double-screened."
    )
...
if run:
    if not api_key:
        ...
        if df.empty:
            ...
            unique_cos = df["company_number"].nunique()
            st.success(str(len(df)) + " founder/acquirer rows across " + str(unique_cos) + " companies.")

            display_df = df.copy()
            display_df["company_profile_link"] = display_df["company_profile_link"].apply(
                lambda u: '<a href="' + u + '" target="_blank">Here</a>' if u else ""
            )
